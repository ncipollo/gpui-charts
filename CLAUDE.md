...
- `src/scrub.rs` + `src/scrub/` — Scrubber options, results, the observable `ScrubState` entity, nearest-sample lookup, aggregation, and the default highlight painter. `scrub/search.rs` holds the `SampleIndex` each plot sorts once so lookups binary search. `Graph` registers the mouse listeners.
